...
@section('title', 'Editar Cuenta Bancaria - ALI3000')
...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-[#000000]">Editar Cuenta Bancaria</h1>
...
<div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 mb-6">
    ...
    <form action="{{ route('admin.datos-bancarios.update', [$consultor->id, $datosBancarios->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" id="banco" name="banco" value="{{ old('banco', $datosBancarios->banco) }}" required
            ...
            <div>
                ...
                <select id="tipo_cuenta" name="tipo_cuenta" required
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#4682B4] focus:ring focus:ring-[#4682B4] focus:ring-opacity-50">
                    <option value="ahorro" {{ old('tipo_cuenta', $datosBancarios->tipo_cuenta) == 'ahorro' ? 'selected' : '' }}>Ahorro</option>
                    <option value="corriente" {{ old('tipo_cuenta', $datosBancarios->tipo_cuenta) == 'corriente' ? 'selected' : '' }}>Corriente</option>
                </select>
                @error('tipo_cuenta')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" id="numero_cuenta" name="numero_cuenta" value="{{ old('numero_cuenta', $datosBancarios->numero_cuenta) }}" required
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#4682B4] focus:ring focus:ring-[#4682B4] focus:ring-opacity-50">
            ...
            <div>
                ...
                <input type="text" id="cedula_rif" name="cedula_rif" value="{{ old('cedula_rif', $datosBancarios->cedula_rif) }}" required
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#4682B4] focus:ring focus:ring-[#4682B4] focus:ring-opacity-50">
                @error('cedula_rif')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" id="titular" name="titular" value="{{ old('titular', $datosBancarios->titular) }}" required
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#4682B4] focus:ring focus:ring-[#4682B4] focus:ring-opacity-50">
                @error('titular')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                <div class="flex items-center h-full">
                    <input type="checkbox" id="es_principal" name="es_principal" class="h-4 w-4 text-[#4682B4] focus:ring-[#4682B4] border-gray-300 rounded" {{ old('es_principal', $datosBancarios->es_principal) ? 'checked' : '' }}>
...
<script>
    ...
    window.addEventListener('load', validarCuenta);
</script>
@endsection