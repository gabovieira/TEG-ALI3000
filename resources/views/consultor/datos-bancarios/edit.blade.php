...
@section('title', 'Editar Datos Bancarios - ALI3000')

@section('page-title', 'Editar Datos Bancarios')
...
<div class="mb-6">
    <div class="flex items-center">
        ...
        <div>
            <h1 class="text-2xl font-bold text-[#000000]">Editar Datos Bancarios</h1>
            <p class="text-[#708090]">Actualiza la información de tu cuenta bancaria</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
    <form action="{{ route('consultor.datos-bancarios.update', $datosBancarios->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select id="banco" name="banco" 
                    ...
                    @foreach($bancos as $banco)
                        <option value="{{ $banco }}" {{ (old('banco', $datosBancarios->banco) == $banco) ? 'selected' : '' }}>
                            {{ $banco }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="tipo_cuenta" name="tipo_cuenta" 
                    ...
                    @foreach($tiposCuenta as $valor => $nombre)
                        <option value="{{ $valor }}" {{ (old('tipo_cuenta', $datosBancarios->tipo_cuenta) == $valor) ? 'selected' : '' }}>
                            {{ $nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" id="numero_cuenta" name="numero_cuenta" 
                       value="{{ old('numero_cuenta', $datosBancarios->numero_cuenta) }}"
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#4682B4] focus:ring focus:ring-[#4682B4] focus:ring-opacity-50" 
            ...
            <div>
                ...
                <input type="text" id="cedula_rif" name="cedula_rif" 
                       value="{{ old('cedula_rif', $datosBancarios->cedula_rif) }}"
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#4682B4] focus:ring focus:ring-[#4682B4] focus:ring-opacity-50" 
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" id="titular" name="titular" 
                       value="{{ old('titular', $datosBancarios->titular) }}"
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#4682B4] focus:ring focus:ring-[#4682B4] focus:ring-opacity-50" 
        ...
        <div>
            ...
            <textarea id="observaciones" name="observaciones" rows="3"
                      class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#4682B4] focus:ring focus:ring-[#4682B4] focus:ring-opacity-50" 
                      placeholder="Información adicional sobre esta cuenta (opcional)">{{ old('observaciones', $datosBancarios->observaciones) }}</textarea>
            @error('observaciones')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        @if($datosBancarios->es_principal)
            <div class="bg-green-50 border border-green-200 rounded-md p-4">
                <div class="flex items-center">
                    ...
                    <span class="text-sm font-medium text-green-800">Esta es tu cuenta principal</span>
                </div>
                <p class="text-sm text-green-700 mt-1">Los pagos se realizarán a esta cuenta por defecto.</p>
            </div>
        @endif
    ...
    <div class="flex items-start">
        ...
        <div class="ml-3">
            ...
            <div class="mt-2 text-sm text-blue-700 space-y-1">
                <p>• Verifica que todos los datos sean correctos antes de actualizar</p>
                <p>• Los cambios se aplicarán inmediatamente</p>
                <p>• Si cambias datos importantes, notifica al administrador</p>
                <p>• Esta cuenta se usará para futuros pagos si es la principal</p>
            </div>
        </div>
    </div>
</div>
